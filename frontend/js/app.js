...
class App {
  ...
  initEventListeners() {
    ...
    if (btnRefresh) btnRefresh.addEventListener('click', () => this.fetchNextRecommendation());
  ...
  resetSession() {
    ...
    this.updateCounters();
    const chips = document.getElementById('history-chips-container');
    if (chips) chips.innerHTML = '<span class="chip-empty">Session reset. Ready for next track...</span>';
  ...
  async fetchNextRecommendation() {
    try {
      ...
      if (data.recommendations && data.recommendations.length > 0) {
        ...
        // Auto-play next stream if player was playing
  ...
  renderCurrentTrack(track) {
    ...
    const spotifyQuery = encodeURIComponent(`${track.title} ${track.artist_name}`);
    const spotifyUrl = track.spotify_url || `https://open.spotify.com/search/${spotifyQuery}`;
    
    const btnOpenSpotify = document.getElementById('btn-open-spotify');
    if (btnOpenSpotify) btnOpenSpotify.href = spotifyUrl;

    const directLink = document.getElementById('spotify-direct-link');
    if (directLink) directLink.href = spotifyUrl;
  ...
  renderQueue(queueTracks, qDist) {
    ...
    if (!queueTracks || queueTracks.length === 0) {
      container.innerHTML = '<p style="color:var(--text-secondary); font-size:12px;">Queue empty.</p>';
      return;
    }

    // Build a genre diversity map from queue tracks
    const genreCounts = {};
    queueTracks.forEach(t => { genreCounts[t.genre] = (genreCounts[t.genre] || 0) + 1; });
    const uniqueGenres = Object.keys(genreCounts).length;

    container.innerHTML = queueTracks.map((t, idx) => {
      // FIX: Q-score indexing — qDist[idx] not qDist[idx+1]; qDist[0] is the current track
      const qVal = qDist && qDist[idx] ? qDist[idx].q_score.toFixed(2) : (2.5 - idx * 0.4).toFixed(2);
      const spotifyUrl = t.spotify_url || `https://open.spotify.com/search/${encodeURIComponent(t.title + ' ' + t.artist_name)}`;
      const coverUrl = t.image_url || DEFAULT_COVER_ART;

      // Recommendation reason chip generation based on acoustic features
      const reason = this._getRecommendationReason(t, idx, uniqueGenres);
      const isDiversePick = t.genre !== (this.currentTrack ? this.currentTrack.genre : t.genre);

      return `
        <div class="queue-item" style="animation-delay: ${idx * 60}ms">
          <div style="display:flex; align-items:center; min-width:0; flex:1;">
            <img class="queue-thumb" src="${coverUrl}" alt="Cover">
            <div class="queue-item-info">
              <span class="queue-item-title">${t.title}</span>
              <span class="queue-item-artist">${t.artist_name} • ${t.genre}</span>
              <span class="queue-reason-chip">${reason}</span>
            </div>
          </div>
          <div class="queue-item-meta">
            <button class="btn-small" onclick="window.app.playSongDirectly('${t.song_id}')" title="Play Now">
              <i class="fa-solid fa-play"></i>
            </button>
            <button class="btn-small" onclick="window.app.openRelatedHubFromSong('${t.song_id}')" title="Find Related Songs">
              <i class="fa-solid fa-wand-magic-sparkles"></i>
            </button>
            <a href="${spotifyUrl}" target="_blank" class="btn-small" style="color:#1ed760;" title="Listen on Spotify">
              <i class="fa-brands fa-spotify"></i>
            </a>
            <div class="q-score-badge" title="Critic Q-value estimate">
              <span class="q-val-text">Q: +${qVal}</span>
              <div class="q-score-bar"><div class="q-score-fill" style="width:${Math.min(100, parseFloat(qVal) / 5 * 100)}%"></div></div>
            </div>
          </div>
        </div>
      `;
   ...
   * Based on queue position, acoustic features, and diversity.
   */
  ...
  async handleFeedback(actionType, rewardDelta) {
    ...
    try {
      ...
      // Sync mini player after feedback
      this.updateMiniPlayer();
  ...
  addHistoryChip(track, action, reward) {
    ...
    const empty = container.querySelector('.chip-empty');
    if (empty) empty.remove();

    const chip = document.createElement('div');
    chip.className = `history-chip ${action === 'liked' ? 'liked' : (action.startsWith('skip') ? 'skipped' : 'listened')}`;
    
    let icon = 'fa-check';
    if (action === 'liked') icon = 'fa-heart';
    if (action === 'skip_early') icon = 'fa-forward-step';
    if (action === 'disliked') icon = 'fa-thumbs-down';
  ...
  async loadCatalog() {
    ...
    try {
      ...
      tbody.innerHTML = songs.map(s => {
        ...
        const spotifyUrl = s.spotify_url || `https://open.spotify.com/search/${encodeURIComponent(s.title + ' ' + s.artist_name)}`;
  ...
  renderSeedSpotlight(seed) {
    ...
    const spotifyUrl = seed.spotify_url || `https://open.spotify.com/search/${encodeURIComponent(seed.title + ' ' + seed.artist_name)}`;
    ...
    container.innerHTML = `
      ...
      <div class="seed-actions">
        ...
        <a href="${spotifyUrl}" target="_blank" class="btn-card-icon spotify" title="Listen on Spotify">
          <i class="fa-brands fa-spotify"></i>
        </a>
      </div>
    `;
  ...
  renderRelatedCard(song) {
    const spotifyUrl = song.spotify_url || `https://open.spotify.com/search/${encodeURIComponent(song.title + ' ' + song.artist_name)}`;
        ...
        return `
          <div class="mini-related-card">
            ...
            <div style="display:flex; gap:6px; margin-top:6px;">
              ...
              <a href="${spotifyUrl}" target="_blank" class="btn-card-icon spotify" style="padding:4px 8px; font-size:10.5px;">
                <i class="fa-brands fa-spotify"></i>
              </a>
            </div>
          </div>
